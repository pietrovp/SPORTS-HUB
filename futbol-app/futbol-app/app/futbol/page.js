...
export default function Home() {
  ...
  useEffect(() => {
    async function cargarDatos() {
      ...
      // 1. Verificamos la sesión directamente en el navegador
      const { data: { user } } = await supabase.auth.getUser();
      setIsUserLoggedIn(!!user);

      // 2. Consultamos los partidos Y la imagen de la sede (El JOIN clave)
      ...
      if (partidosError) {
        console.error("Error al cargar partidos en Home:", partidosError);
      } else if (partidosData) {
        // 3. Consultamos las inscripciones para contar cupos ocupados
        ...
        // Mapeamos los cupos ocupados a cada partido
  ...
  // --- COMPONENTE REUTILIZABLE: CÓMO FUNCIONA ---
  ...
  return (
    <div className="flex flex-col gap-12 max-w-6xl mx-auto pb-12">
      ...
      <div className="relative w-full min-h-[500px] md:min-h-[600px] bg-[#0B0C15] rounded-[2.5rem] overflow-hidden flex flex-col justify-center items-center text-center shadow-2xl border border-gray-800/50">
        <img 
          src="https://images.unsplash.com/photo-1574629810360-7efbbe195018?q=80&w=1893&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D" 
          alt="Jugador de fútbol en la cancha" 
        ...
        <div className="relative z-10 max-w-3xl flex flex-col items-center px-6 mt-12 md:mt-0">
          <span className="mb-4 px-4 py-1.5 rounded-full bg-white/10 backdrop-blur-md border border-white/20 text-white/90 text-xs font-bold uppercase tracking-widest shadow-lg">
            La cancha te espera
          </span>
          <h1 className="text-5xl md:text-7xl font-black text-white uppercase tracking-tighter leading-[0.9] drop-shadow-2xl">
            Juega fútbol <br />
            <span className="text-[#00FF9D] drop-shadow-[0_0_25px_rgba(0,255,157,0.4)]">cuando quieras</span>
          </h1>
          <p className="mt-6 text-gray-300 text-sm md:text-lg max-w-lg font-medium drop-shadow-md">
          ...
          <div className="mt-10 flex gap-4">
            <Link
              href="#partidos"
              className="px-8 py-4 bg-[#00FF9D] text-[#0B0C15] font-black uppercase tracking-widest rounded-full text-sm hover:bg-[#00e58d] transition-all transform hover:scale-105 shadow-[0_0_40px_rgba(0,255,157,0.4)]"
            >
              Ver partidos
            </Link>
          </div>
      ...
      {!cargando && (
        <>
          {/* --- CÓMO FUNCIONA (SÓLO SI NO HA INICIADO SESIÓN) --- */}
              ...
              {proximos.length === 0 && (
                <div className="col-span-2 bg-white rounded-3xl p-12 text-center border border-gray-200 shadow-sm">
                  <span className="text-gray-300 mb-3 block">
                    <svg className="w-12 h-12 mx-auto" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg>
                  </span>
              ...
              {proximos.map((partido) => (
                <PartidoCard
                  ...
                  partido={{
                    ...
                    // AQUÍ ESTÁ LA MAGIA: Buscamos la imagen en la tabla relacional sedes
                ...
                jugados.map((partido) => (
                  ...
                  >
                    ...
                    <div className="pl-4">
                      ...
                      <h3 className="font-black text-white text-xl leading-none uppercase">
                        {partido.cancha_lugar || partido.cancha || partido.titulo || "Cancha"}
                      </h3>
          ...
          {/* --- CÓMO FUNCIONA (AL FONDO SÓLO SI SÍ HA INICIADO SESIÓN) --- */}
          {isUserLoggedIn && (
            <div className="pt-12 mt-4 border-t border-gray-200/60">
              {comoFuncionaSection}
            </div>
          )}
        </>
