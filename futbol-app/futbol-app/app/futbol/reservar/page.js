...
export default function AdminCanchasDashboard() {
  ...
  // 2. Cargar Franjas y Partidos Reservados de forma SEGURA
  useEffect(() => {
    async function cargarDatosSede() {
      ...
      // --- CORRECCIÓN: Búsqueda segura de Partidos Reales ---
      const { data: partidosData, error: pError } = await supabase
        .from("partidos")
        .select("*") // Quitamos el Join directo que rompía la consulta
        .eq("sede_id", sedeActiva.id)
        .eq("estado", "abierto")
      ...
      if (partidosData && partidosData.length > 0) {
        ...
        // Buscamos los jugadores inscritos en estos partidos
  ...
  async function cancelarPartidoPorGerente(partido) {
    ...
    try {
      ...
      if (inscripciones && inscripciones.length > 0) {
        // En partidos privados invitados entran con 0, pero si es creador pagó el costo completo.
        // Lo más seguro es que el sistema devuelva lo que cada quien pagó viendo el historial, pero por ahora devolvemos el precio base.
        const costo = partido.precio_creditos ?? 1;

        for (const inscripcion of inscripciones) {
          const userId = inscripcion.user_id;
          
          // Si el partido es privado y es el creador, se le devuelve el total. Si no es el creador, no se le devuelve nada porque entró gratis
          let montoADevolver = costo;
          if (partido.tipo_acceso === "privado") {
             if (userId === partido.creador_id) {
                 // Aquí deberíamos buscar en franjas cuánto costaba la cancha, pero por seguridad, leemos el ledger. 
                 // Por simplificar, le devolvemos un estimado (puedes ajustar esta lógica según tu negocio)
                 montoADevolver = partido.cupos_totales || 14; 
             } else {
                 montoADevolver = 0; // Entró gratis
             }
          }
