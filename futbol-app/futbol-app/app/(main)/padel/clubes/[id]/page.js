...
export default function PublicClubDetailPage() {
  ...
  async function cargarDetalleClub() {
    try {
      ...
      const canchasPadel = (courtsData || []).filter(c => c.sport_type === "padel" || (!c.sport_type && c.sport_type !== "futbol"));
  ...
  const abrirModalTurno = async (cancha, bloque, precioCalculado) => {
    ...
    if (lockExistente) {
      return mostrarNotificacion(
        ...
        "Esta pista ya está en proceso de reserva (por ti en otra pestaña o por otro usuario/recepción).", 
        "warning"
      );
    }
          ...
          <div className="bg-white rounded-3xl p-5 sm:p-6 max-w-md w-full shadow-2xl space-y-4 max-h-[90vh] overflow-y-auto [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden" onClick={(e) => e.stopPropagation()}>
            ...
            {pasoModal === 1 && (
              <div className="space-y-4">
                ...
                <button onClick={() => setPasoModal(2)} className="w-full py-3.5 bg-slate-900 text-[#00FF9D] font-black text-xs uppercase tracking-widest rounded-2xl shadow-md cursor-pointer font-sans transition-all hover:bg-slate-800">
                  Continuar al Pago →
                </button>
              </div>
            )}
